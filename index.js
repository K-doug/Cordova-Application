...
function TaxiShare() {
    ...
    var platform = new H.service.Platform({
        ...
        apikey: "QjwmJsrm8tKTKvykZ2834jIfflJTCrQ823Vc2TQW0qA",
    });
    ...
    function addMarkerToMap(address) {
        if (address) {
            ...
            var onSuccess = function (data) {
                ...
                //only run function if the data provided is in the correct format else throw error
                if (data.length > 0 && data[0].lat && data[0].lon) {
                    
                    //create a marker for each entry and add it to an array
                    var marker = new H.map.Marker({lat: data[0].lat, lng: data[0].lon});
                    markers.push(marker);
                    
                    //loop through the marker array and add each marker to the map
                    markers.forEach(function (marker) {
                        map.addObject(marker);
                        console.log("Marker added to map");
                    });
                } else {
                    console.error("Invalid data received from Nominatim API");
                }
            };
    ...
    function updateMap() {
        ...
        //declare the hire address variable and ensure that the address provided is correct from entry
        var userid = getInputValue("userid", "user1");
        var hire_address = getInputValue("location", "Milton Keynes Central Station");
        console.log(hire_address);
        
        //remove any markers currently showing on the map
        clearMarkersFromMap();
        
        function onMatchSuccess(obj) {
          console.log("match/list: received obj", obj);
        
          if (obj.status == "success") {
            
            //declare a variable for the matches and inform user how many matches have been found/
            //console log created to ensure data has been provided by AJAX
            var matches = obj.data;
            console.log(matches);
            alert("Matching " + matches.length + " orders");

            // Loop through each match found and retrieve the address for each of the entries
            matches.forEach(function(match) {
              var matchUrl = BASE_URL + "matches/" + match.id + "?userid=" + userid;
              console.log("Matches: Sending GET to " + matchUrl);
              $.ajax(matchUrl, {
                type: "GET",
                data: { hire_address: match.userid },
                success: function() {
                // store the hire address into a variable and pass the variable into the hire address function
                  var address = match.hire_address;
        ...
        //AJAX call to retrieve the matches available for the offer or request
        var lstUrl = BASE_GET_URL + "matches?userid=" + userid;
        console.log("match/list: Sending GET to " + lstUrl);
        $.ajax(lstUrl, { type: "GET", data: {}, success: onMatchSuccess });
      }
    ...
    function offer(userid, address, startTime, endTime) {
        // TODO 2(a) FR1.1
        // You need to implement this function
        console.log(startTime)// See the TMA for an explanation of the functional requirements
        function onSuccess(obj) {
            console.log("offer: received obj", obj);

            // Inform the user what happened
            if (obj.status == "success") {
                alert("Offer of user " + userid + " has been successfully posted.");
            } else if (obj.message) {
                alert(obj.message);
            } else {
                alert("Invalid userid: " + userid);
        ...
        $.ajax(url, { type: "POST", data: { userid: userid, address: address, start: startTime, end: endTime, type: 0 }, success: onSuccess });
        updateMap();
    }
    ...
    function request(userid, address, startTime) {
        ...
        function onSuccess(obj) {
            ...
            // Inform the user what happened
            if (obj.status == "success") {
                alert("User " + userid + " has been successfully posted.");
            } else if (obj.message) {
                alert(obj.message);
            } else {
                alert("Invalid userid: " + userid);
        ...
        $.ajax(url, { type: "POST", data: { userid: userid, address: address, start: startTime, type: 1 }, success: onSuccess });
    }
    ...
    this.offerTaxi = function () {
        ...
        var userid = getInputValue("userid", "user1");
        var address = getInputValue("location", "Milton Keynes Central Station");
        var startTime = getInputValue("time", defaultStartTime); // eg. 2020:12:18:14:38
        var hours = getInputValue("duration", "1"); // duration in hours
    ...
    this.requestTaxi = function () {
        // TODO adjust the following to get the required data from your HTML view
        var userid = getInputValue("userid", "user1");
        var address = getInputValue("location", "Open University, Milton Keynes");
